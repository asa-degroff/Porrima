...
- **Memory scoring**: `rrf_score * recency_decay * (importance / 10)` with RRF combining vector search and FTS5 full-text search rankings; 30-day half-life on recency. Context estimation uses LLM-reported `usage.totalTokens` as baseline, char-based as fallback.
