...
- **Mid-turn compaction**: Detects 85% context usage during tool loops, breaks the agent loop, archives + compacts, injects handoff message (progress summary + tool call log), resumes via `agentLoopContinue`. Multi-cycle (up to 5) for long-running tasks. Strips all trailing assistant messages before resume.
