...
- `allowTruncation` **disables the rebase**: a flagged save is array-authoritative and deletes what its array omits, so only writers that intentionally truncate may pass it. Today that is only the `/edit` route, and it is a guarded transaction — every save passes `rejectOnRevisionConflict`, checked under the write lock before any write; a revision movement since the load throws `RevisionConflictError` and the route refuses with 409 (reload-and-retry) instead of deleting a concurrent append. Compaction never removes rows (`_outOfContext` marks + summary splice), so compaction-then-save sites must NOT pass the flag — they rebase around concurrent appends.
