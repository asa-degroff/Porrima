...
- A loaded row the DB no longer has (a concurrent truncating writer deleted it) is dropped by the rebase, not re-emitted as new — resurrecting it would silently undo the truncation. Counted as `truncatedDropped` in the rebase log.
- `allowTruncation` **disables the rebase**: a flagged save is array-authoritative and deletes what its array omits, so only writers that intentionally truncate may pass it. Today that is only the `/edit` route's truncation save, and it passes `rejectOnRevisionConflict` — checked under the write lock before any write; a revision movement since the load throws `RevisionConflictError` and the route refuses with 409 (reload-and-retry, nothing written). The route's later saves (pre-send compaction, memory delta) are unflagged and rebase, with the edited row resolved by `_rowId`, so a concurrent append survives instead of forcing a 409 after the truncation already landed. Compaction never removes rows (`_outOfContext` marks + summary splice), so compaction-then-save sites must NOT pass the flag — they rebase around concurrent appends.
