...
**Status:** implemented 09-09 (PR1–PR4; PR5 docs). Companion to [cross-chat-messaging.md](../cross-chat-messaging.md) v5, phase **P0b**. Tests: `server/src/__tests__/chat-storage-revision.test.ts` (22 cases) plus the full server suite (684 passing). Deferred: skipping no-op metadata-only saves; a route-level test for current-message tracking (`§5.5`) — the storage-level drift test covers the reconciliation. Line numbers re-verified 09-10 00:5x against `git fe703ab` — the /stop fix renumbered `chat.ts` (by +6 to +213 depending on region) and touched `compaction.ts`, `system-chat.ts`, `automation-runner.ts`, `chat-turn-runner.ts`, `turn-compaction.ts`; the §5 audit below was re-derived from content at that commit. If `chat.ts` moves again, re-derive from the content anchors (the distinctive code quoted at each site), not the stale numbers.
...
- **Keep it on — the `/edit` route, the only truncating writer:** the truncation save (after "Truncate everything from targetIndex onwards") and the pre-send-compaction save (`preCompactionFlushHook(chat, "pre-send flush failed (edit)")`), both paired with `rejectOnRevisionConflict` (below). The delta save passes `rejectOnRevisionConflict` *without* the flag — it does not truncate.
- **Dropped from — compaction-then-save sites (must rebase):** `chat.ts` mid-turn compaction (after the `midTurnFirstKeptSequence` capture), the `/compact` confirmation save (after `chat.messages.push(confirmMsg)`), resume pre-send (`preCompactionFlushHook(chat, "pre-send flush failed (resume)")`), send pre-send (`preCompactionFlushHook(chat, "pre-send flush failed")`); `system-chat.ts` synthesis + wake pre-compaction; `automation-runner.ts` pre-compaction (the P2 wake path — the site that made this urgent); `chat-turn-runner.ts` headless mid-turn compaction; `turn-compaction.ts` end-of-turn compaction. Compaction never removes rows (`_outOfContext` marks + summary splice), so these saves must preserve concurrent appends; the flag's only remaining job there would be to suppress a shrinkage warning that never fires.

**`/edit` revision guard (spec commitment, implemented 09-10):** `saveChat` gains `rejectOnRevisionConflict` — checked under the write lock, before any write, independent of the rebase kill switch: if the stored revision moved past `chat._baseRevision`, throw `RevisionConflictError` (nothing written). The `/edit` route passes it on every save, making the route a guarded transaction: any concurrent write since the load refuses the edit (409, reload-and-retry) instead of the truncation deleting the concurrent append — or, in the reverse interleave, a later rebase resurrecting the rows the truncation deleted. The old length guard (`chat.messages.length < dbRowCount`, 500) remains as the corruption backstop.
...
- **Audit residual `length - 1` assumptions** in the send/resume/edit paths during PR4 (`rg "messages\.length - 1"`), classifying each as "last is the current message" or not.
