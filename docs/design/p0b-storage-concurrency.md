...
### 5.4 `allowTruncation` is the truncation marker

Keep it on: `chat.ts:3479, 4815, 5033, 5626, 5699`; `turn-compaction.ts:203`.
The audit found compaction-then-save sites that omit it but do not currently remove rows (compaction marks `_outOfContext` and splices a summary; `truncateChatHistory` verified 09-10 to not shrink the array): `chat.ts:4535`, `system-chat.ts:997, 1339`, `automation-runner.ts:256`, `chat-turn-runner.ts:986`. Add the flag for intent clarity; the rebase makes omitting it non-lossy either way. The flag is currently log-only (`chat-storage.ts:1626-1642`) and stays log-only in P0b.

### 5.5 Route and reader fixes

- **Current-message tracking (send route):** after pushing the user row and saving (`chat.ts:4938`/`:4946`), capture its `_rowId` and resolve `currentUserIndex` (`chat.ts:5122-5125`) and `trailingRow`/`sendTimeAnchor` (`:5184-5186`) by that id, not by `messages.length - 1`. After an interleaved rebase the writer's message is no longer last, and the current code would treat the other request's row as the current prompt (wrong context split and time anchor). `splitNextUserContext`'s `persistedHistoryEnd` semantics (`chat.ts:799-825`) must be validated with a route-level test that simulates the interleave; if same-chat concurrent POSTs turn out to be unreachable (the route may coalesce them into steering/follow-ups), keep the fix as defense-in-depth and pin it.
...
9. **Truncation unchanged:** `/edit`-style `allowTruncation: true` still deletes from the target down.
10. **Window guard:** `saveChat` on a `getChatWithWindow` result refuses.
11. **Route current-message tracking:** simulate the interleaved array ([origins, user2, user1]) and assert the route resolves/computes `currentUserIndex` and `trailingRow` from the captured id, not the tail (may live in `chat-turn-runner.test.ts` or a route test).
