...
for (const entry of needsBackfill) {
  ...
  try {
    ...
    // Decode JXL to RGBA using icodec
    jxl.loadDecoder();
    const rgba = jxl.decode(jxlBuffer);
    
    // Convert RGBA to PNG using sharp
    pngBuffer = await sharp(Buffer.from(rgba.data.buffer, rgba.data.byteOffset, rgba.data.byteLength), {
      raw: {
        width: rgba.width,
        height: rgba.height,
        channels: 4
    ...
    console.log(`  ✓ Converted JXL → PNG (${rgba.width}x${rgba.height})`);
  } catch (err) {
