...
const DEFAULT_PANEL_CLASS = "left-0 right-0 top-full mt-1";
...
export function Dropdown({
  ...
  // Depth is shared chrome, not part of the replaceable trigger classes. Only
  // opt in bordered triggers; borderless custom triggers remain flat.
  const triggerClasses = triggerClassName.split(/\s+/);
  const hasBorder = triggerClasses.includes("border") && !triggerClasses.includes("border-0");
  const triggerClass = hasBorder ? `depth-raised relative ${triggerClassName}` : triggerClassName;
  ...
  return (
    <div className={wrapperClass} ref={state.ref}>
      ...
      <DropdownPanel open={state.open} className={panelClassName} contentClassName={panelContentClassName}>
        {children}
      </DropdownPanel>
    </div>
  );
