      ...
      <div ref={rowRef} className={`flex flex-row items-start max-w-[95%] md:max-w-[80%] min-w-0`}>
        ...
        <div className={`flex flex-col ${isUser ? "items-end" : "items-start"} min-w-0`}>
          <div
            ...
            className={`relative rounded-2xl px-3 md:px-4 py-3 max-w-full min-w-0 overflow-x-hidden ${
              isUser
                ? "text-white/95"
                : "text-white/90"
          ...
          >
            ...
            {/* Read aloud button - bottom right corner of bubble */}
            {!isUser && showReadAloudButton && (
              <button
                ...
                className={`absolute bottom-2 right-2 flex h-7 w-7 items-center justify-center rounded-md opacity-0 transition-all duration-150 group-hover:opacity-100 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-white/30 ${
                  isPlayingTts
                    ? "cursor-not-allowed bg-white/[0.04] text-white/35"
                    : "text-white/35 hover:bg-white/10 hover:text-white/75"
                }`}
                title={isPlayingTts ? "Audio is playing" : "Read aloud"}
                aria-label={isPlayingTts ? "Audio is playing" : "Read aloud"}
              >
                {isPlayingTts ? (
                  <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="animate-pulse">
                    <rect x="6" y="4" width="3" height="16" />
